...
class Problem(DeviceComponent):
    ...
    @log_enter('[DEBUG] Updating problem {self.full_name(device)} ...')
    @log_return('Found problem {self.full_name(device)}.', condition='ret')
    @log_and_ignore_error('Failed to update problem {self.full_name(device)}.', exc_info=True)
